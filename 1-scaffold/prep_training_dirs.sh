...
# the release step (1-scaffold/release.jac) emits into the train/valid DIRECTORY shape mlx_lm_lora
# actually reads.
#
# WHY THIS EXISTS. Both arms' configs point `data:` at a DIRECTORY, not a file:
#
#     2-train/run_sft_spectrum.sh / stock/run_sft.sh   --data "$EXP/dataset/sft"
#     2-train/dpo/run_dpo_spectrum.sh / stock/run_dpo_nofuse.sh  --data "$EXP/dataset/dpo"
#
...
#     [EXP=experiments/<exp>] 1-scaffold/prep_training_dirs.sh   # EXP default: experiments/08-nitin-new2-ds
...
EXP="${EXP:-experiments/08-nitin-new2-ds}"; export EXP   # nan_guard.jac reads it too
...
if [ -z "$SPLIT_ONLY" ] || [ "$SPLIT_ONLY" = "sft" ]; then
  ...
  if [ -f "$EXP/dataset/sft/train.jsonl" ]; then
    ...
    jac run 1-scaffold/nan_guard.jac
  fi
fi
