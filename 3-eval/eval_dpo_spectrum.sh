...
RDIR="${RDIR:-$EXP/dpo/results}"
# Where run_dpo_spectrum.sh wrote its run state. RDIR is redirected per holdout
# (holdout (b) runs set e.g. RDIR=$EXP/dpo/results/holdoutB),
...
TRAIN_RDIR="${TRAIN_RDIR:-$EXP/dpo/results}"
...
JAC_EVAL_MODE=mlx JAC_EVAL_MODEL="$BASE_MODEL" JAC_EVAL_ADAPTER="$DPO_ADAPTER" \
  ...
  jac run 3-eval/eval_functional.jac | tee "$RDIR/final_last.txt"
...
if [ -f "$BEST_ADAPTER/adapters.safetensors" ] && [ "$BEST_STEP" != "$FINAL_STEP" ]; then
  ...
  JAC_EVAL_MODE=mlx JAC_EVAL_MODEL="$BASE_MODEL" JAC_EVAL_ADAPTER="$BEST_ADAPTER" \
    ...
    jac run 3-eval/eval_functional.jac | tee "$RDIR/final_best.txt"
else
...
echo "stock-dpo arm, and cross-phase vs the lineage table in README.md."
