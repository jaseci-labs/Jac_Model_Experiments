...
# chat-template fix. The driver is 2-train/dpo/dpo_spectrum_train.jac, which carries
...
#
# LAUNCH ONLY AFTER 2-train/run_sft_spectrum.sh AND 3-eval/eval_sft_spectrum.sh have completed.
# ----------------------------------------------------------------------------
set -euo pipefail
...
cd "$(cd "$(dirname "$0")/../.." && pwd)"   # repo root (this file: 2-train/dpo/)
...
SPEC_DIR="2-train/spectrum"
DRIVER="2-train/dpo/dpo_spectrum_train.jac"
...
DPO_CFG="2-train/configs/dpo_lora.yaml"
...
if [ ! -f "$SFT_ADAPTER/adapters.safetensors" ]; then
  ...
  echo "    Run 2-train/run_sft_spectrum.sh (and 3-eval/eval_sft_spectrum.sh) first."
  exit 1
fi
...
while true; do
  ...
  JAC_EVAL_MODE=mlx JAC_EVAL_MODEL="$BASE_MODEL" JAC_EVAL_ADAPTER="$SNAP" \
    ...
    jac run 3-eval/eval_functional.jac 2>&1 | tail -5 | tee -a "$RDIR/train.log"
  STEP_PCT="$(python3 -c "
import json
p = 0.0
try:
...
echo "Next: 3-eval/eval_dpo_spectrum.sh"
