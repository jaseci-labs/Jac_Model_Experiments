...
# EXP = the experiment dir (relative to the repo root); data is read from and
# adapter/results are written under it.
...
CFG="2-train/configs/sft_spectrum.yaml"
...
while true; do
  ...
  if [ "$REMAIN" -le 0 ] && [ -f "$ADAPTER_FILE" ]; then
    done_mark train
    echo "=== spectrum SFT training done ($DONE_STEPS/$TOTAL_ITERS). Next: 3-eval/eval_sft_spectrum.sh ==="
    break
  fi
  ...
  while kill -0 "$SEG_PID" 2>/dev/null; do
    ...
    JAC_TRAIN_LOG="$RDIR/.segment.log" JAC_METRICS="/dev/null" JAC_PLOT_DIR="$RDIR" \
      jac run 3-eval/plot_metrics.jac >/dev/null 2>&1 || true
  done
