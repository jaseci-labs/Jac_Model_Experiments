...
#
# Outputs: adapters/dpo-on-sft-fresh-spectrum{,-best}, results/dpo-spectrum/.
...
DRIVER="$FRESH_SPEC_DIR/dpo_spectrum_train.py"        # shared, unduplicated
LAYERS="$SPEC_DIR/configs/spectrum_layers.json"
CFGFIX="$FRESH_SPEC_DIR/adapter_config_fix.py"        # shared, unduplicated
BASE_MODEL="models/qwen-q4"
SFT_ADAPTER="model-experiments/04-cpt-sft/sft_cptspectrum_probe/adapters/sft-on-cptspectrum"
DPO_ADAPTER="model-experiments/04-cpt-sft/sft_cptspectrum_probe/adapters/dpo-on-sft-cptspectrum"
BEST_ADAPTER="model-experiments/04-cpt-sft/sft_cptspectrum_probe/adapters/dpo-on-sft-cptspectrum-best"
RDIR="model-experiments/04-cpt-sft/sft_cptspectrum_probe/results/dpo-spectrum"
...
DATA="model-experiments/04-cpt-sft/sft_cptspectrum_probe/dataset/dpo"
DPO_CFG="model-experiments/04-cpt-sft/sft_cptspectrum_probe/configs/dpo_lora.yaml"
...
best_step = -1
try:
    with open('model-experiments/04-cpt-sft/sft_cptspectrum_probe/results/sft-spectrum/metrics_functional.jsonl') as f:
        for line in f:
            line = line.strip()
...
if [ ! -f "$DRY_DONE_MARK" ]; then
  ...
  python "$DRIVER" --spectrum-layers "$LAYERS" --model "$BASE_MODEL" --train --data "$DATA" \
    ...
    --adapter-path model-experiments/04-cpt-sft/sft_cptspectrum_probe/adapters/dpo-dry-spectrum \
