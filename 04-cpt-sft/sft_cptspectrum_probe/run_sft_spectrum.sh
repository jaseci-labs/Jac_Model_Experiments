...
DRIVER="model-experiments/04-cpt-sft/sft_fresh_probe/spectrum/spectrum_lora_layers.py"  # shared, unduplicated
...
for f in model-experiments/04-cpt-sft/sft_cptspectrum_probe/dataset/sft/train.jsonl \
         model-experiments/04-cpt-sft/sft_cptspectrum_probe/dataset/sft/valid.jsonl \
         "$CFG" "$DRIVER"; do
...
if [ ! -f "$ADAPTER_FILE" ] && ! is_done dry && [ "${SKIP_DRY:-0}" != "1" ]; then
  ...
  python "$DRIVER" --spectrum-layers "$LAYERS" --config "$CFG" --iters "$DRY_ITERS" \
    --adapter-path "model-experiments/04-cpt-sft/sft_cptspectrum_probe/adapters/dry-spectrum" 2>&1 | tail -25
