...
for A in "$DPO_ADAPTER" "$BEST_ADAPTER"; do
  ...
  python "$FRESH_SPEC_DIR/adapter_config_fix.py" --adapter-dir "$A" --spectrum-layers "$LAYERS" \
  ...
  PYTHONPATH="$FRESH_SPEC_DIR:${PYTHONPATH:-}" python - "$A" <<'PY' | tee -a "$RDIR/key_assertion.txt"
import sys
