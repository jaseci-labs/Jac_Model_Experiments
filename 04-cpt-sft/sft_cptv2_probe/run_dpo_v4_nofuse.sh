#!/usr/bin/env bash
# ============================================================================
# ROOT-CAUSE FIX for the DPO collapse, ported to the cptv2 arm. See
# sft_fresh_probe/run_dpo_nofuse.sh for the full diagnosis: `mlx_lm.fuse` on
# an int4-quantized model silently destroys the fine-grained SFT LoRA delta
# on re-quantization (~15% of packed weight bits change, but the resulting
# "fused" model generates functionally identical to raw base). v1/v2/v3 all
# fused first -- every one of them trained DPO on top of an effectively
# un-SFT'd base, independent of hyperparameters.
#
# THE FIX (same as the fresh arm): skip the fuse, train directly against
# models/qwen-q4, seed DPO's LoRA from the SFT adapter via
# --resume-adapter-file. ALSO carries the fresh arm's independently-found
# add_generation_prompt chat-template fix (dpo_fixed_train.py, copied
# unmodified from sft_fresh_probe/ -- same package, same bug, applies here
# too) since both are real bugs worth fixing together in one clean run.
#
# New output paths (adapters/dpo-on-sft-v4-nofuse{,-best}, results/dpo-v4-nofuse/)
# -- v1/v2/v3 results untouched.
# ----------------------------------------------------------------------------
set -euo pipefail
...
best_step = -1
try:
    with open('model-experiments/04-cpt-sft/sft_cptv2_probe/results/sft/metrics_functional.jsonl') as f:
        for line in f:
            line = line.strip()
...
if [ ! -f "$DRY_DONE_MARK" ]; then
  ...
  python "$DRIVER" --model "$BASE_MODEL" --train --data model-experiments/04-cpt-sft/sft_cptv2_probe/dataset/dpo \
    --train-mode dpo --config model-experiments/04-cpt-sft/sft_cptv2_probe/configs/dpo_lora.yaml \
    --adapter-path model-experiments/04-cpt-sft/sft_cptv2_probe/adapters/dpo-v4-dry-nofuse \
