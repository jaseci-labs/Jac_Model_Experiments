#!/usr/bin/env bash
# DPO-on-SFT-on-CPT-v2 runner, v3: same v1 hyperparams (beta=0.1, lr=1e-6,
# 250 iters) as this arm's original run, but ported from the fresh-probe
# arm's improved instrumentation so both arms can be compared on an
# identical methodology this time -- val_batches=10, per-segment snapshot +
# inline subset functional eval, best-snapshot tracking, and a collapse gate
# that RECORDS where it would have fired but does not stop training (full
# 250-iter budget always runs, per explicit user request to see the whole
# curve and find the real sweet spot with complete information). v1/v2's
# own results (adapters/dpo-on-sft, dpo-on-sft-v2) are untouched -- this is
# a new, separate run.
set -euo pipefail
...
# SFT baseline for the collapse gate -- read from this arm's real SFT result.
# Filters to full-holdout rows (total==855) and takes the highest step among
# THOSE (same fix as the fresh arm's script -- avoids picking a noisier
# interim subset row instead of the true final).
...
if [ ! -f "$DRY_DONE_MARK" ]; then
  ...
  python -m mlx_lm_lora.train --model "$SFT_FUSED" --train --data model-experiments/04-cpt-sft/sft_cptv2_probe/dataset/dpo \
    ...
    --adapter-path model-experiments/04-cpt-sft/sft_cptv2_probe/adapters/dpo-v3-dry \
