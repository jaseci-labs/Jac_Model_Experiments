...
#   - new output paths (adapters/dpo-on-sft-nofuse{,-best}, results/dpo-nofuse/)
...
DPO_ADAPTER="model-experiments/04-cpt-sft/sft_fresh_probe/adapters/dpo-on-sft-nofuse"
BEST_ADAPTER="model-experiments/04-cpt-sft/sft_fresh_probe/adapters/dpo-on-sft-nofuse-best"
RDIR="model-experiments/04-cpt-sft/sft_fresh_probe/results/dpo-nofuse"
...
if [ ! -f "$DRY_DONE_MARK" ]; then
  ...
  python "$DRIVER" --model "$BASE_MODEL" --train --data model-experiments/04-cpt-sft/sft_fresh_probe/dataset/dpo \
    ...
    --adapter-path model-experiments/04-cpt-sft/sft_fresh_probe/adapters/dpo-dry-nofuse \
