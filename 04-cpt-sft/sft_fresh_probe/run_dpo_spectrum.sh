...
SFT_ADAPTER="model-experiments/04-cpt-sft/sft_fresh_probe/adapters/sft-on-fresh-spectrum"
...
DATA="model-experiments/04-cpt-sft/sft_fresh_probe/dataset/dpo"
DPO_CFG="model-experiments/04-cpt-sft/sft_fresh_probe/configs/dpo_lora.yaml"
...
best_step = -1
try:
    with open('model-experiments/04-cpt-sft/sft_fresh_probe/results/sft-spectrum/metrics_functional.jsonl') as f:
        for line in f:
            line = line.strip()
...
if [ ! -f "$DRY_DONE_MARK" ]; then
  ...
  python "$DRIVER" --spectrum-layers "$LAYERS" --model "$BASE_MODEL" --train --data "$DATA" \
    ...
    --adapter-path model-experiments/04-cpt-sft/sft_fresh_probe/adapters/dpo-dry-spectrum \
