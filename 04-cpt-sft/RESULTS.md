...
**Bottom line:** CPT-v2 gives the raw base model a real, large head start (+37pp functional pass rate before any training) — but that advantage does not survive as a statistically confirmed edge once SFT trains on top (+2.8pp, p≈0.20), and DPO collapses both arms to the same noise-band floor (~12%) regardless of CPT. Run to their full training budgets with no early stopping and searched checkpoint-by-checkpoint for a better stopping point, neither arm reveals a hidden advantage the headline numbers miss. **A fresh base put through the identical SFT recipe performs indistinguishably from the CPT-v2 arm at every post-training stage.**
