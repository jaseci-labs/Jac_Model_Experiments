...
# the release step ($EXP/scaffold/release.jac) emits into the train/valid DIRECTORY shape mlx_lm_lora
# actually reads.
#
# WHY THIS EXISTS. Both arms' configs point `data:` at a DIRECTORY, not a file:
#
#     $EXP/train/run_sft_spectrum.sh / stock/run_sft.sh   --data "$EXP/dataset/sft"
#     $EXP/train/dpo/run_dpo_spectrum.sh / stock/run_dpo_nofuse.sh  --data "$EXP/dataset/dpo"
#
...
# release.jac writes single release files instead:
#
...
#
# The Nitin holdout ($EXP/dataset/nitin_holdout.jsonl) is carved upstream in Stage 0
# and is NOT touched here -- this script only splits what release.jac released.
#
# USAGE (run from anywhere; the experiment is the dir this script sits in):
#     experiments/<exp>/scaffold/prep_training_dirs.sh
#     FORCE=1 ... # overwrite existing non-empty splits
#     SPLIT_SEED=42 TRAIN_FRAC=0.85 ...  # both overridable
# ----------------------------------------------------------------------------
set -euo pipefail
EXP_ABS="$(cd "$(dirname "$0")/.." && pwd)"   # <repo>/experiments/<exp> (this file: <exp>/scaffold/)
...
FORCE="${FORCE:-0}"
# SPLIT_ONLY=sft|dpo restricts this run to one track. Needed because the two
# releases finish at different times: the SFT release can be final and ready to
# train while the DPO collect is still running its dual compile gate, and
# splitting a half-written dpo_train.jsonl would silently produce a truncated
# training set. Default (unset) does both, exactly as before.
...
if [ -z "$SPLIT_ONLY" ] || [ "$SPLIT_ONLY" = "sft" ]; then
  ...
  if [ -f "$EXP/dataset/sft/train.jsonl" ]; then
    ...
    jac run "$EXP/scaffold/nan_guard.jac"
  fi
fi
