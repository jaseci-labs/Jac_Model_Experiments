...
# $EXP/eval/eval_functional.jac unmodified (env-var driven), exactly
...
jac run "$SPEC_DIR/adapter_config_fix.jac" --adapter-dir "$ADAPTER" --spectrum-layers "$LAYERS" \
...
# NOTE (jac port): the heredoc below imports `adapter_config_fix` from $SPEC_DIR --
...
JAC_EVAL_MODE=mlx JAC_EVAL_MODEL=models/qwen-q4 JAC_EVAL_ADAPTER="" \
  ...
  jac run "$EXP/eval/eval_functional.jac" | tee "$RDIR/base.txt"

TMPADP="$EXP/tmp/sft-spectrum-ckpt-eval"
...
for CK in "$CKPT_DIR"/*_adapters.safetensors; do
  ...
  JAC_EVAL_MODE=mlx JAC_EVAL_MODEL=models/qwen-q4 JAC_EVAL_ADAPTER="$TMPADP" \
    ...
    jac run "$EXP/eval/eval_functional.jac" 2>/dev/null | tail -5
done
rm -rf "$TMPADP"

echo ">>> final spectrum SFT checkpoint -- FULL holdout"
TOTAL_ITERS="$(grep -E '^iters:' "$EXP/train/configs/sft_spectrum.yaml" | grep -oE '[0-9]+' | head -1)"
JAC_EVAL_MODE=mlx JAC_EVAL_MODEL=models/qwen-q4 JAC_EVAL_ADAPTER="$ADAPTER" \
  ...
  jac run "$EXP/eval/eval_functional.jac" | tee "$RDIR/final.txt"

echo "=== functional eval sweep done: $METRICS ==="
echo "Next: paired McNemar vs this phase's OWN stock arm ($EXP/stock/results/final.txt),"
echo "and cross-phase vs the lineage table in README.md (04-cpt-sft 639/855 is the one to beat)."
