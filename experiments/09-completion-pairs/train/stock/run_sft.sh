...
CFG="$EXP/train/configs/sft_stock.yaml"
...
if [ ! -f "$ADAPTER_FILE" ] && ! is_done dry && [ "${SKIP_DRY:-0}" != "1" ]; then
  ...
  mlx_lm.lora --config "$CFG" --data "$DATA" --iters "$DRY_ITERS" \
    --adapter-path "$EXP/tmp/dry-stock" 2>&1 | tail -25
...
while true; do
  ...
  if [ "$DONE_STEPS" -gt 0 ] && [ -f "$ADAPTER_FILE" ]; then
    mlx_lm.lora --config "$CFG" --data "$DATA" --adapter-path "$ADAPTER" --iters "$ATTEMPT_ITERS" \
