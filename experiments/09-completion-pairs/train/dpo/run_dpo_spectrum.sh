...
# chat-template fix. The driver is $EXP/train/dpo/dpo_spectrum_train.jac, which carries
...
#
# Outputs: $EXP/dpo/{adapter,adapter-best,results}/.
# Nothing existing is touched.
#
# LAUNCH ONLY AFTER $EXP/train/run_sft_spectrum.sh AND $EXP/eval/eval_sft_spectrum.sh have completed.
# ----------------------------------------------------------------------------
set -euo pipefail
...
EXP_ABS="$(cd "$(dirname "$0")/../.." && pwd)"   # <repo>/experiments/<exp> (this file: <exp>/train/dpo/)
...
DRIVER="$EXP/train/dpo/dpo_spectrum_train.jac"
LAYERS="$SPEC_DIR/spectrum_layers.json"
CFGFIX="$SPEC_DIR/adapter_config_fix.jac"
BASE_MODEL="models/qwen-q4"
SFT_ADAPTER="$EXP/adapter"
DPO_ADAPTER="$EXP/dpo/adapter"
BEST_ADAPTER="$EXP/dpo/adapter-best"
RDIR="$EXP/dpo/results"
...
DATA="$EXP/dataset/dpo"
DPO_CFG="$EXP/train/configs/dpo_lora.yaml"
...
if [ ! -f "$SFT_ADAPTER/adapters.safetensors" ]; then
  ...
  echo "    Run $EXP/train/run_sft_spectrum.sh (and $EXP/eval/eval_sft_spectrum.sh) first."
  exit 1
fi
...
if ! is_done verify_patches; then
  ...
  jac run "$DRIVER" --verify-patches --spectrum-layers "$LAYERS" 2>&1 | tee "$RDIR/verify_patches.txt"
...
# NOTE (nitin-new retarget): the lookup below filters on `total == 855`, i.e. it
...
best_step = -1
try:
    with open('${EXP}/results/metrics_functional.jsonl') as f:
        for line in f:
            line = line.strip()
...
if [ ! -f "$DRY_DONE_MARK" ]; then
  ...
  jac run "$DRIVER" --spectrum-layers "$LAYERS" --model "$BASE_MODEL" --train --data "$DATA" \
    ...
    --adapter-path "$EXP/tmp/dpo-dry-spectrum" \
...
while true; do
  ...
  jac run "$CFGFIX" --adapter-dir "$SNAP" --spectrum-layers "$LAYERS" \
  ...
  JAC_EVAL_MODE=mlx JAC_EVAL_MODEL="$BASE_MODEL" JAC_EVAL_ADAPTER="$SNAP" \
    ...
    jac run "$EXP/eval/eval_functional.jac" 2>&1 | tail -5 | tee -a "$RDIR/train.log"
  STEP_PCT="$(python3 -c "
import json
p = 0.0
try:
...
echo "Next: $EXP/eval/eval_dpo_spectrum.sh"
