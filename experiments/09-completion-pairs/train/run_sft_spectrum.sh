...
set -euo pipefail
# NOTE (jac port): the training/eval drivers are .jac now and are launched with
...
# EXP = the experiment dir this script sits in (<exp>/train/), relative to the
# repo root; data is read from and adapter/results are written under it.
EXP_ABS="$(cd "$(dirname "$0")/.." && pwd)"   # <repo>/experiments/<exp>
cd "$EXP_ABS/../.."                           # repo root
EXP="experiments/$(basename "$EXP_ABS")"
[ -d ".venv/bin" ] && export PATH="$PWD/.venv/bin:$PATH"

SPEC_DIR="$EXP/train/spectrum"
DRIVER="$SPEC_DIR/spectrum_lora_layers.jac"
LAYERS="$SPEC_DIR/spectrum_layers.json"
CFG="$EXP/train/configs/sft_spectrum.yaml"
DATA="$EXP/dataset/sft"               # passed as --data (overrides the yaml's data:)
ADAPTER="$EXP/adapter"
...
# 09: continue training FROM 08's finished adapter. INIT_ADAPTER is only ever READ --
# passed as --resume-adapter-file to the dry run and to the first attempt of a fresh
# start (no 09 adapter yet). Every write goes to $ADAPTER or $EXP/tmp. Once 09's own
# adapter + progress exist, the stock resume branch below takes over unchanged.
INIT_ADAPTER="${INIT_ADAPTER:-experiments/08-nitin-new2-ds/adapter/adapters.safetensors}"

need() { command -v "$1" >/dev/null 2>&1 || { echo "MISSING: $1"; exit 1; }; }
need jac; need python
for f in "$DATA/train.jsonl" \
         "$DATA/valid.jsonl" \
         "$CFG" "$DRIVER" "$INIT_ADAPTER"; do
  [ -f "$f" ] || { echo "MISSING: $f"; exit 1; }
done
# INIT_ADAPTER must live outside this experiment, so no path this script writes can alias it.
INIT_DIR_ABS="$(cd "$(dirname "$INIT_ADAPTER")" && pwd)"
case "$INIT_DIR_ABS/" in
  "$EXP_ABS/"*) echo "!!! INIT_ADAPTER ($INIT_ADAPTER) is inside $EXP -- refusing (it must be read-only input)."; exit 1 ;;
esac
if [ ! -f "$LAYERS" ]; then
  echo "MISSING: $LAYERS"
  echo "  The layer selection is produced by the SNR scan, not by this script."
  echo "  Run PLAYBOOK.md step 1.2 first:"
...
# mlx_lm loads --resume-adapter-file with load_weights(strict=False): keys for layers the
# driver did not LoRA-wrap would be dropped SILENTLY. So INIT_ADAPTER's layer set must equal
# spectrum_layers.json. Read from the safetensors header only (stdlib, no model load).
python - "$INIT_ADAPTER" "$LAYERS" <<'PY' || { echo "!!! INIT_ADAPTER layer set != $LAYERS -- not training."; exit 1; }
import json, re, struct, sys
with open(sys.argv[1], "rb") as f:
    hdr = json.loads(f.read(struct.unpack("<Q", f.read(8))[0]))
got = sorted({int(m.group(1)) for k in hdr for m in [re.search(r"\.layers\.(\d+)\.", k)] if m})
want = sorted(json.load(open(sys.argv[2]))["layers"])
print(f"INIT_ADAPTER layers {got} vs spectrum_layers.json {want}")
...
if [ ! -f "$ADAPTER_FILE" ] && ! is_done dry && [ "${SKIP_DRY:-0}" != "1" ]; then
  ...
  jac run "$DRIVER" --spectrum-layers "$LAYERS" --config "$CFG" --data "$DATA" --iters "$DRY_ITERS" \
    --adapter-path "$EXP/tmp/dry-spectrum" --resume-adapter-file "$INIT_ADAPTER" 2>&1 | tail -25
...
if [ -f "$ADAPTER_FILE" ] && [ ! -f "$PROGRESS_FILE" ] && ! is_done train; then
  echo "!!! $ADAPTER_FILE exists (a finished adapter) but no run markers."
...
while true; do
  ...
  if [ "$REMAIN" -le 0 ] && [ -f "$ADAPTER_FILE" ]; then
    done_mark train
    echo "=== spectrum SFT training done ($DONE_STEPS/$TOTAL_ITERS). Next: $EXP/eval/eval_sft_spectrum.sh ==="
    break
  fi
  ...
  if [ "$DONE_STEPS" -gt 0 ] && [ -f "$ADAPTER_FILE" ]; then
    jac run "$DRIVER" --spectrum-layers "$LAYERS" --config "$CFG" --data "$DATA" --adapter-path "$ADAPTER" \
      --iters "$ATTEMPT_ITERS" --resume-adapter-file "$ADAPTER_FILE" >> "$RDIR/.segment.log" 2>&1 &
  else
    # fresh start (no 09 progress/adapter yet): initialise from 08's adapter (read only)
    jac run "$DRIVER" --spectrum-layers "$LAYERS" --config "$CFG" --data "$DATA" --adapter-path "$ADAPTER" \
      --iters "$ATTEMPT_ITERS" --resume-adapter-file "$INIT_ADAPTER" >> "$RDIR/.segment.log" 2>&1 &
  fi
  SEG_PID=$!
  ...
  while kill -0 "$SEG_PID" 2>/dev/null; do
    ...
    JAC_TRAIN_LOG="$RDIR/.segment.log" JAC_METRICS="/dev/null" JAC_PLOT_DIR="$RDIR" \
      jac run "$EXP/eval/plot_metrics.jac" >/dev/null 2>&1 || true
  done
