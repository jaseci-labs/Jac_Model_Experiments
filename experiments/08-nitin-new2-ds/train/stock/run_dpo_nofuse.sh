...
EXP_ABS="$(cd "$(dirname "$0")/../.." && pwd)"   # <repo>/experiments/<exp> (this file: <exp>/train/stock/)
...
best_step = -1
try:
    with open('${EXP}/stock/results/metrics_functional.jsonl') as f:
        for line in f:
            line = line.strip()
...
if [ ! -f "$DRY_DONE_MARK" ]; then
  ...
  jac run "$DRIVER" --model "$BASE_MODEL" --train --data "$EXP/dataset/dpo" \
    --train-mode dpo --config "$DPO_CFG" \
    --adapter-path "$EXP/tmp/dpo-dry-stock" \
