...
- **Run everything from the repo root**: the directory that contains
  `experiments/08-nitin-new2-ds`, `models/` and `.venv/`.
...
with its drop reason). Full funnel: `experiments/08-nitin-new2-ds/docs/corpus-triage-report.md`.
...
`experiments/08-nitin-new2-ds/docs/funnel/stats.json` sum correctly stage-to-stage. (summarized in `corpus-triage-report.md`.)
...
`experiments/08-nitin-new2-ds/stock/results/{train.log,metrics_functional.jsonl}`. This ran to
