...
ADAPTER="model-experiments/06-nitin-ds-sft/spectrum_probe/adapters/sft-on-nitin-spectrum"
CKPT_DIR="$ADAPTER/checkpoints"
RDIR="model-experiments/06-nitin-ds-sft/spectrum_probe/results/sft-spectrum"
...
for f in model-experiments/06-nitin-ds-sft/dataset/sft/train.jsonl \
         model-experiments/06-nitin-ds-sft/dataset/sft/valid.jsonl \
         "$CFG" "$DRIVER"; do
...
if [ ! -f "$ADAPTER_FILE" ] && ! is_done dry && [ "${SKIP_DRY:-0}" != "1" ]; then
  ...
  python "$DRIVER" --spectrum-layers "$LAYERS" --config "$CFG" --iters "$DRY_ITERS" \
    --adapter-path "model-experiments/06-nitin-ds-sft/spectrum_probe/adapters/dry-spectrum" 2>&1 | tail -25
