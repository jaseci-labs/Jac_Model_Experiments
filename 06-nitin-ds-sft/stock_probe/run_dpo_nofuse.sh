...
# fix via dpo_fixed_train.py -- both fixes are real and both are kept):
...
DRIVER="model-experiments/06-nitin-ds-sft/stock_probe/dpo_fixed_train.py"
BASE_MODEL="models/qwen-q4"
SFT_ADAPTER="model-experiments/06-nitin-ds-sft/stock_probe/adapters/sft-on-nitin"
...
RDIR="model-experiments/06-nitin-ds-sft/stock_probe/results/dpo-nofuse"
...
best_step = -1
try:
    with open('model-experiments/06-nitin-ds-sft/stock_probe/results/sft/metrics_functional.jsonl') as f:
        for line in f:
            line = line.strip()
...
if [ ! -f "$DRY_DONE_MARK" ]; then
  ...
  python "$DRIVER" --model "$BASE_MODEL" --train --data model-experiments/06-nitin-ds-sft/dataset/dpo \
    --train-mode dpo --config model-experiments/06-nitin-ds-sft/stock_probe/configs/dpo_lora.yaml \
    --adapter-path model-experiments/06-nitin-ds-sft/stock_probe/adapters/dpo-dry-nofuse \
