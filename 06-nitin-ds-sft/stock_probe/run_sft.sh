...
CFG="model-experiments/06-nitin-ds-sft/stock_probe/configs/sft.yaml"
ADAPTER="model-experiments/06-nitin-ds-sft/stock_probe/adapters/sft-on-nitin"
CKPT_DIR="$ADAPTER/checkpoints"    # true-global-step-named snapshots, see the loop comment below for why
RDIR="model-experiments/06-nitin-ds-sft/stock_probe/results/sft"
...
for f in model-experiments/06-nitin-ds-sft/dataset/sft/train.jsonl \
         model-experiments/06-nitin-ds-sft/dataset/sft/valid.jsonl "$CFG"; do
...
if [ ! -f "$ADAPTER_FILE" ] && ! is_done dry && [ "${SKIP_DRY:-0}" != "1" ]; then
  ...
  mlx_lm.lora --config "$CFG" --iters "$DRY_ITERS" \
    --adapter-path "model-experiments/06-nitin-ds-sft/stock_probe/adapters/dry" 2>&1 | tail -25
