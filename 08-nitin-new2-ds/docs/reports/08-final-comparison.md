...
1. **SFT train loss silently went NaN, `mlx_lm.lora` does not crash on it.**
   ...
   (since deleted). The check is now `scripts/nan_guard.jac`, run by
...
4. **Scope-cut race.** After the DPO-scope-cut decision, the orchestrator
   ...
   out of `stock_probe/adapters/`), so zero GPU time was actually spent on
...
- `spectrum_probe/adapters/sft-on-nitin-spectrum/` — final adapter (8200/8200 iters)
- `spectrum_probe/results/sft-spectrum/` — `train.log`, `verify_layers.txt`,
...
- `spectrum_probe/results/sft-spectrum-holdoutB/` — `adapter_config_rewrite.txt`,
...
1. If RQ1 becomes relevant again, retrain the stock arm
   (`stock_probe/run_sft.sh`); the stock adapter no longer exists.
