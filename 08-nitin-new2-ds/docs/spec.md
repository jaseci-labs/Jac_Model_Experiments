...
- The 7-source merge pipeline (`scripts/pipeline.jac`) — not a port, a new
...
- `scripts/copy_holdout.jac` — copies 07's holdout verbatim and re-verifies
  leakage against it; 07 had no equivalent since it carved its own holdout
  fresh.
- `scripts/release.jac` — pool-schema → trainer-schema splitter; 07's
