...
# chat-template fix. The driver is spectrum/dpo_spectrum_train.jac, which carries
...
SFT_ADAPTER="model-experiments/07-nitin-ds-new-sft/spectrum_probe/adapters/sft-on-nitin-spectrum"
...
RDIR="model-experiments/07-nitin-ds-new-sft/spectrum_probe/results/dpo-spectrum"
...
DATA="model-experiments/07-nitin-ds-new-sft/dataset/dpo"
DPO_CFG="model-experiments/07-nitin-ds-new-sft/spectrum_probe/configs/dpo_lora.yaml"
...
best_step = -1
try:
    with open('model-experiments/07-nitin-ds-new-sft/spectrum_probe/results/sft-spectrum/metrics_functional.jsonl') as f:
        for line in f:
            line = line.strip()
...
if [ ! -f "$DRY_DONE_MARK" ]; then
  ...
  jac run "$DRIVER" --spectrum-layers "$LAYERS" --model "$BASE_MODEL" --train --data "$DATA" \
    ...
    --adapter-path model-experiments/07-nitin-ds-new-sft/spectrum_probe/adapters/dpo-dry-spectrum \
