# We're trying to teach a 30B model a language it has never read. Here's every dead end so far

*Blog 4 of 4 on the model work behind Jac Model Studio. The other three go deep on [continual pretraining](../model-work/post.md), [layer selection](../fine-tuning-layers-breakthrough/post.md), and the app (that post lives in the JMS repo under `jms/docs/blog/jms-model-studio/`). This one is the map: what we're trying to do, everything we've tried, and where it stands as of early August 2026.*
...
## 01. Pick a base model, and prove you picked it
...
![Graph holdout pass rate at base, SFT, and DPO, across the bake-off field](images/bakeoff_graph_holdout.png)
...
## 02. The RL detour, and three weeks of measuring the wrong thing
...
![The corrected RL numbers, 11% to 94%, mostly by fixing the ruler](images/rl_corrected_journey.png)
...
![CPT-v2 against its three pre-registered acceptance gates, 0 of 3 cleared](images/cptv2_acceptance_gauges.png)
...
![Four completed arms on the same frozen 855-row holdout](images/four_arms_summary.png)

Four arms done, all on that same holdout, all directly comparable:

- CPT gives the raw base a **+37pp** head start (10.5% → 47.3%) that SFT almost entirely absorbs (+2.8pp, p≈0.20, not significant).
- Spectrum layer selection significantly beats the default on a fresh base at every stage (SFT +4.9pp p=0.023, DPO-final +10.6pp p<0.0001).
- Spectrum shows no significant difference once CPT-v2 is already in the chain (SFT −2.1pp p=0.34, DPO-best an exact tie).

That last line is the honest, annoying one, and it's why arm 5 exists.

## Arm 5, paused at leg 1 of 12
...
- Five stale-path bugs fixed in the CPT orchestration code, including one that would have silently overwritten CPT-v2's own irreplaceable per-leg evidence. All 65 existing plus 5 new tests pass.
- Gate 1, layer verification: 48 LoRA-switched linears, exactly 281.838M trainable params.
- Gate 2, a 2-iteration dry leg: **30.058 GB peak memory**, roughly 6GB under this machine's ceiling. The biggest risk in the plan, since Spectrum's picks include block 0 and backprop therefore traverses all 48 blocks at a 4096-token sequence length.
- Gate 3, forgetting check on the dry adapter: 16/16, with CPT-v2's historical evidence verified untouched.
